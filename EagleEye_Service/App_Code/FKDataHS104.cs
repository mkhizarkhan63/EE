...
namespace FKDataHS104
{
    // 为数字工号的workcode记录数据定义  未动工 //TODO
  /*
  typedef struct tagGLOG_DATA_CIF12_1
  {
  } GLOG_DATA_CIF11;  // size = 12 byte

  typedef struct tagUSER_ID_CIF12_1 {
  } USER_ID_CIF11;  // size = 8 byte
    
    
  typedef struct tagGENERALLOGDATA
  {
  ...
  #ifdef CONFIG_WORKCODE
    unsigned char        WorkCode;
    unsigned char        Reseved[3];
  #endif
  } GENERALLOGDATA, *P_GENERALLOGDATA;  // 12byte + 4
  ...
  */
  ...
  public class UserIdInfo
  {
    ...
    public void GetBackupNumberEnrolledFlag(out byte[] abytEnrolledFlag)
    {
      ...
      int pvcnt = flagEnrolled[sctPalmCount];
