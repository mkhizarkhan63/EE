...
namespace FKDataHS200
{
    // 3760c기대에서 사용하는 로그자료에서 InOut마당을 2비트에서 5비트로  BackupNumber를 8비트에서 5비트로 변환한  자료클라스이다.
    //{ 다음의 2개의 구조체는 하오쑨 3760c RTBIO기대에서 리용하는 구조체들이다.
    /*
        typedef struct tagGLOG_DATA_CIF1
        {
	        unsigned char	Valid;				// 1
	        unsigned char	Second;				// 1	

	        unsigned short	Kind  : 6;
	        unsigned short	InOut : 5;			// 
	        unsigned short	BackupNumber : 5;	// 2
	
	        unsigned long 	User_ID;			// 4

	        unsigned short	Year    : 12;		
	        unsigned short	Month	: 4;		// 2
	        unsigned short	Day		: 5;
	        unsigned short	Hour	: 5;
	        unsigned short	Minute	: 6;		// 2
        }__attribute__((packed)) GLOG_DATA_CIF1; /* Length = 12byte  
    ...
    */
    ...
    public class GLog
    {
       ...
       // public byte VerifyMode;
        ...
        internal static readonly BitVector32.Section sctInOut = BitVector32.CreateSection((1 << 5) - 1, sctKind);
        internal static readonly BitVector32.Section sctBackupNumber = BitVector32.CreateSection((1 << 5) - 1, sctInOut);
        ...
        public GLog(byte[] abytLog)
        {
            ...
            Vaild = abytLog[0];
            Second = abytLog[1];
            tmMode = new BitVector32(BitConverter.ToInt16(abytLog, 2));
        ...
        public void GetLogData(out byte[] abytLog)
        {
            ...
            abytLog[1] = Second;
            Buffer.BlockCopy(
                    BitConverter.GetBytes((short)tmMode.Data), 0,
                    abytLog, 2,
                    2);
            //abytLog[2] = 0;
